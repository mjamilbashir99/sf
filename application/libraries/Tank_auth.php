...
class Tank_auth
{
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'group_id' => '', 'status' => ''));
	...
	{
		return $this->ci->users->get_user_by_id($this->get_user_id());
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation, $group_id)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			$is_email_activation = ($email_activation)?FALSE:TRUE;
			if (!is_null($res = $this->ci->users->create_user($data, $is_email_activation))) {
	...
	/**
	 * Update user password (only when user is logged in)
	 *
	 * @param	string
	 * @param	string
	 * @return	bool
